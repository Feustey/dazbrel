...
async fn test_mcp_client_response_times() {
    ...
    assert!(
        recommendations_duration < Duration::from_millis(300),
        "Recommendations took too long: {:?}",
        recommendations_duration
    );
    println!(
    ...
    assert!(
        analysis_duration < Duration::from_millis(600),
        "Performance analysis took too long: {:?}",
        analysis_duration
    );
    println!(
...
async fn test_concurrent_request_performance() {
    ...
    for _ in 0..100 {
        ...
        tasks.push(tokio::spawn(
            async move { client_clone.health_check().await },
        ));
    }
...
async fn test_large_payload_performance() {
    ...
    println!(
        "Large payload serialization time: {:?}",
    ...
    println!(
        "Large payload deserialization time: {:?}",
...
async fn test_error_recovery_performance() {
    ...
    println!(
        "Error recovery test: {} successes, {} failures in {:?}",
...
async fn test_api_rate_limiting_simulation() {
    ...
    println!(
        "Rate limiting test: {} successes, {} rate limited in {:?}",
