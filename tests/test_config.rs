...
impl Default for TestConfig {
    fn default() -> Self {
        Self {
            ...
            test_node_pubkey: "02a1b2c3d4e5f6789abcdef123456789abcdef123456789abcdef123456789abcdef".to_string(),
...
pub mod test_data {
    use dazno_umbrel::api::mcp_client::{NodeMetrics, ChannelMetrics, ActionResult, MCPRecommendation, ActionType, Priority};
    use chrono::Utc;
    use uuid::Uuid;
    use serde_json::json;
    ...
    pub fn create_test_recommendation(action_type: ActionType, priority: Priority) -> MCPRecommendation {
        MCPRecommendation {
    ...
    pub fn create_test_action_result(success: bool) -> ActionResult {
        ActionResult {
            ...
            success,
            message: if success { "Test action completed successfully" } else { "Test action failed" }.to_string(),
            timestamp: Utc::now(),
        }
    }
...
pub mod test_utils {
    ...
    pub fn is_valid_pubkey(pubkey: &str) -> bool {
        ...
        pubkey.chars().skip(2).all(|c| c.is_ascii_hexdigit())
    }
}

/// Test assertions and validators
pub mod test_assertions {
    use dazno_umbrel::api::mcp_client::{NodeMetrics, MCPRecommendation, ActionType, Priority};
    use serde_json::Value;
    
    pub fn assert_valid_node_metrics(metrics: &NodeMetrics) {
        assert!(!metrics.pubkey.is_empty(), "Node pubkey should not be empty");
        assert!(!metrics.alias.is_empty(), "Node alias should not be empty");
        assert!(metrics.wallet_balance > 0, "Wallet balance should be positive");
        assert!(metrics.total_capacity >= metrics.channel_balance, "Total capacity should be >= channel balance");
        
        for channel in &metrics.channels {
            assert!(!channel.channel_id.is_empty(), "Channel ID should not be empty");
            assert!(!channel.peer_pubkey.is_empty(), "Peer pubkey should not be empty");
            assert!(channel.capacity > 0, "Channel capacity should be positive");
            assert!(channel.local_balance + channel.remote_balance <= channel.capacity, 
                   "Channel balances should not exceed capacity");
            assert!(channel.uptime_percentage >= 0.0 && channel.uptime_percentage <= 100.0,
    ...
    pub fn assert_valid_recommendation(rec: &MCPRecommendation) {
        ...
        assert!(!rec.description.is_empty(), "Recommendation description should not be empty");
        assert!(rec.expected_roi_impact.is_finite(), "ROI impact should be finite");
        
        match rec.action_type {
            ActionType::AdjustFees => {
                assert!(rec.parameters.get("channel_id").is_some(), "Fee adjustment should have channel_id");
            },
            ActionType::OpenChannel => {
                assert!(rec.parameters.get("peer_pubkey").is_some(), "Open channel should have peer_pubkey");
                assert!(rec.parameters.get("suggested_capacity").is_some(), "Open channel should have capacity");
            },
            ActionType::CloseChannel => {
                assert!(rec.parameters.get("channel_id").is_some(), "Close channel should have channel_id");
            },
            ActionType::RebalanceChannel => {
                assert!(rec.parameters.get("amount_sats").is_some(), "Rebalance should have amount");
            },
        }
    }
    
    pub fn assert_valid_performance_analysis(analysis: &Value) {
        assert!(analysis.get("performance_metrics").is_some(), "Should have performance metrics");
        assert!(analysis.get("competitive_analysis").is_some(), "Should have competitive analysis");
        assert!(analysis.get("insights").is_some(), "Should have insights");
        assert!(analysis.get("predictions").is_some(), "Should have predictions");
        
        let metrics = analysis.get("performance_metrics").unwrap();
        assert!(metrics.get("current_roi_percentage").is_some(), "Should have ROI percentage");
        assert!(metrics.get("routing_success_rate").is_some(), "Should have success rate");
        
        let competitive = analysis.get("competitive_analysis").unwrap();
        assert!(competitive.get("vs_amboss_advantage").is_some(), "Should have Amboss comparison");
        assert!(competitive.get("dazno_ml_accuracy").is_some(), "Should have ML accuracy");
        ...
        assert!(predictions.get("roi_30d").is_some(), "Should have 30d ROI prediction");
    }
}
...
mod tests {
    ...
    fn test_utilities() {
        ...
        assert!(is_valid_pubkey("02a1b2c3d4e5f6789abcdef123456789abcdef123456789abcdef123456789abcdef"));
        assert!(is_valid_pubkey("03a1b2c3d4e5f6789abcdef123456789abcdef123456789abcdef123456789abcdef"));
    ...
    async fn test_async_utilities() {
        ...
        let condition_timeout = wait_for_condition(
            || async { false },
